# 🚗 SpeedAuto - Gestão para Concessionárias

![SpeedAuto Banner](https://img.shields.io/badge/SpeedAuto-Gestão_Inteligente-E31E3D?style=for-the-badge)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

## 📖 Sobre o Projeto

**SpeedAuto** é uma plataforma de gestão completa para concessionárias de veículos, desenvolvida para transformar dados confusos em decisões rápidas e colocar sua concessionária na rota do crescimento acelerado.

> *"Onde a gestão corre na mesma velocidade dos seus negócios."*

A lentidão dos sistemas antigos e o caos das planilhas estão freando os lucros das concessionárias. A SpeedAuto nasceu para ser o motor da sua gestão.

## ✨ Funcionalidades

- 📊 **Dashboard em Tempo Real** - Visão completa do negócio
- 🚗 **Estoque de Veículos** - Gerenciamento completo do inventário
- 💰 **Gerenciamento de Vendas** - Acompanhamento de transações e faturamento
- 👥 **Leads** - Controle de clientes e oportunidades
- 💳 **Simulador de Financiamento** - Cálculo transparente de parcelas
- 📈 **Relatórios e Análises** - Dados acionáveis e exportáveis

## 🛠️ Tecnologias

### Front-End
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat&logo=typescript&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![Tailwind](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat&logo=tailwind-css&logoColor=white)

### Back-End
![Node.js](https://img.shields.io/badge/Node.js-43853D?style=flat&logo=node.js&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)

### Ferramentas
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-100000?style=flat&logo=github&logoColor=white)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat&logo=figma&logoColor=white)
![Trello](https://img.shields.io/badge/Trello-0052CC?style=flat&logo=trello&logoColor=white)

## 🚀 Como Usar

### Instalação Local

```bash
# Clone o repositório
git clone https://github.com/vt-debug/speedauto-HTML-fd.git

# Entre no diretório
cd speedauto-HTML-fd

# Abra o index.html no navegador
# ou use um servidor local como Live Server
```

### Deploy

🌐 **Acesse a versão online:** [speedauto.vercel.app](https://speedauto.vercel.app)

## 📚 Documentação

Para documentação completa, consulte o arquivo [DOCUMENTACAO.md](./DOCUMENTACAO.md)

A documentação inclui:
- Visão geral detalhada do projeto
- Estrutura de arquivos e componentes
- Guia de estilos e design
- Explicação de animações e interações
- Instruções de instalação e deploy
- Informações sobre a equipe

## 👥 Equipe de Desenvolvimento

| Nome | Função | GitHub | LinkedIn |
|------|--------|--------|----------|
| **Vitor Alexandre** | Desenvolvedor Full-Stack | [@vt-debug](https://github.com/vt-debug) | [LinkedIn](https://www.linkedin.com/in/vitor-alexandre-356aa42a7) |
| **Lucas Clessa** | Desenvolvedor Full-Stack | [@lukinhasc-dev](https://github.com/lukinhasc-dev) | [LinkedIn](https://www.linkedin.com/in/lukinhasclessa/) |
| **Flávio Henrique** | AI - Developer | [@FhVieira13](https://github.com/FhVieira13) | [LinkedIn](https://www.linkedin.com/in/flavio-henrique-984951223/) |
| **Débora Gonçalves** | Desenvolvedora Front-End | [@Dehhminiqz](https://github.com/Dehhminiqz) | [LinkedIn](https://www.linkedin.com/in/debora-gon%C3%A7alves-de-oliveira-bb0501287/) |
| **Gabriel Augusto** | AI - Developer | [@Gabriel-Augusto05](https://github.com/Gabriel-Augusto05) | [LinkedIn](https://www.linkedin.com/in/augusto1605/) |

## 📞 Contato

**Endereço:** Rua Régis Bittencourt, 157 - Centro, Taboão da Serra - SP - CEP 06768-000  
**Telefone:** (11) 96203-0664  
**E-mail:** WLdeveloper@speedauto.com.br

## 📄 Licença

© 2024 SpeedAuto. Todos os direitos reservados.

Este projeto foi desenvolvido como parte de um trabalho acadêmico, unindo conhecimentos de lógica de programação, desenvolvimento web e back-end.

---

**Desenvolvido com ❤️ pela equipe SpeedAuto**